class module.exports.Release
  ...
  release: (platform, fn) ->
    ...
    switch platform
      when 'android'
        ...
        @fatal 'You must specify a platform for release. Only "android" is currently supported.'
        fn() if fn
  ...
  buildTree: (platform) ->
    @file.mkdir @path.join(@config.releases, platform)
    @
